...
bool SwapChain::Create(DeviceContext* device, int width, int height)
{
    ...
    //
    {
        ...
        if (physicalDeviceInfo.m_vkSurfaceCapabilities.maxImageCount > 0 &&
            imageCount > physicalDeviceInfo.m_vkSurfaceCapabilities.maxImageCount)
        {
            imageCount = physicalDeviceInfo.m_vkSurfaceCapabilities.maxImageCount;
        }

        uint32_t queueFamilyIndices[] = {(uint32_t)device->m_graphicsFamilyIdx, (uint32_t)device->m_presentFamilyIdx};
    ...
    //
    {
        ...
        allocInfo.memoryTypeIndex =
            device->FindMemoryTypeIndex(memRequirements.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    ...
    //
    {
        ...
        {
            VkImageView attachments[2] = {m_vkImageViews[i], m_vkDepthImageView};
...
uint32_t SwapChain::BeginFrame(DeviceContext* device)
{
    ...
    result = vkAcquireNextImageKHR(device->m_vkDevice,
                                   m_vkSwapChain,
